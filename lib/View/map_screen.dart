...
class MapScreen extends StatefulWidget {
  ...
  const MapScreen({super.key, required this.garageLocation, required this.userLocation});
...
class _MapScreenState extends State<MapScreen> {
  ...
  // Add markers for the garage and user location
  void _setMarkers() {
    setState(() {
      _markers.add(Marker(
        markerId: MarkerId('garage'),
        position: widget.garageLocation,
        infoWindow: InfoWindow(title: "Garage Location"),
      ));
  ...
  // Fetch and draw polyline between the garage and user location
  void _getPolyline() async {
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
      googleApiKey: "AIzaSyALNuiCXZUBxsinIpbbCxdKJizY96aZ1cM",
      request: PolylineRequest(
        origin: PointLatLng(widget.userLocation.latitude, widget.userLocation.longitude),
        destination: PointLatLng(widget.garageLocation.latitude, widget.garageLocation.longitude),
        mode: TravelMode.driving,
      ),
    );
  ...
  void _fitToBounds() {
    LatLngBounds bounds = LatLngBounds(
      southwest: LatLng(
        widget.userLocation.latitude < widget.garageLocation.latitude ? widget.userLocation.latitude : widget.garageLocation.latitude,
        widget.userLocation.longitude < widget.garageLocation.longitude ? widget.userLocation.longitude : widget.garageLocation.longitude,
      ),
      northeast: LatLng(
        widget.userLocation.latitude > widget.garageLocation.latitude ? widget.userLocation.latitude : widget.garageLocation.latitude,
        widget.userLocation.longitude > widget.garageLocation.longitude ? widget.userLocation.longitude : widget.garageLocation.longitude,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Garage Location")),
      body: GoogleMap(
        initialCameraPosition: CameraPosition(
          target: widget.garageLocation,
          zoom: 12,
        ),
        markers: _markers,
        polylines: _polylines,
